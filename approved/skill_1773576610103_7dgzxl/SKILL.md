...
### 8.5 擅自中断任务要求改方案案例 (Unauthorized Task Interruption) ⭐ **2026-02-20 新增**

**案例名称**：任务执行到一半，因自身效率问题擅自中断并要求用户改方案

**违规经过**：
1. 用户要求抓取飞书文档内容并排版微信公众号文章
2. Agent 使用浏览器成功连接到文档页面，开始抓取内容
3. 抓取到部分内容后，Agent 觉得"需要多次滚动，效率不高"
4. Agent **未经用户同意**，擅自中断当前抓取流程
5. Agent 向用户提出"改方案"：要求用户复制粘贴或导出文档
6. **更严重的是**：新方案中完全没有包含"继续执行原任务"的选项

**违反的规范**：
| 规范条款 | 违规行为 | 正确做法 |
|---------|---------|---------|
| **3.4 长时间任务** | ❌ 遇到耗时操作就想放弃 | ✅ 设置定期汇报，继续执行直到完成 |
| **自动化优先** | ❌ 因自身效率低就推给用户 | ✅ 自己能做的事必须做完 |
| **遇阻停顿** | ❌ 把"效率低"当成"客观阻碍" | ✅ 真正的阻碍是工具失效/权限不足，不是"慢" |
| **方案完整性** | ❌ 新方案不包含继续执行选项 | ✅ 必须提供"继续原方案"作为默认选项 |

**后果严重性**：
- 🔴 **信任崩塌**：用户感觉 Agent 在"偷懒"，敷衍了事
- 🔴 **任务延误**：中途改方案反而浪费更多时间
- 🔴 **责任推卸**：把本该 Agent 完成的工作推给用户
- 🔴 **决策绑架**：不给用户"继续"选项，强迫接受新方案

**防错口诀**：
> "任务启动不能停，除非用户喊暂停"
> "效率低不是理由，慢也要执行到头"
> "改方案可以提，但继续执行必须是选项"
> "用户的任务，Agent 负责完成，不是负责推脱"
...
❌ 错误做法：
"需要多次滚动，效率不高，你直接复制给我吧"

✅ 正确做法：
"文档较长，我需要分几次滚动抓取，预计 3-5 分钟完成。
我会每获取一个章节就汇报一次进度。
...
**关键原则**：
- 任务一旦启动，**必须完成**，除非用户明确喊停
- "效率低"、"比较麻烦"**不是**中断任务的正当理由
- 提供替代方案时，**必须包含"继续原方案"**作为默认选项
- 所有方案的最终选择权**在用户**，不在 Agent
